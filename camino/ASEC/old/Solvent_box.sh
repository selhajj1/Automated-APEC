...
while [[ $pairs -lt 0 ]]; do
   ...
   if [[ $charge -ne 0 ]]; then
      ...
      echo ""
    #   read pairs
        pairs=$(grep 'number_pairs_NaCl' "$PARENT_DIR/parameters"| awk '{print $2}')
   fi
...
#
   ...
   while [[ $res -eq 0 ]]; do
      ...
      if [[ $charge -lt 0 ]]; then
         ...
         $gropath/gmx genion -seed $seed -s ions.tpr -p ${Project}_box_sol.top -pname NA -pq 1 -np $pcharge -nname CL -nq -1 -nn $pairs -o ${Project}_box_sol_ion.gro 2> addedions << EOF
15
EOF
...
#         ../../../update_infos.sh "Added_Ions" "${pcharge}_NA" ../../../Infos.dat
      ...
      if [[ $charge -ge 0 ]]; then
         ...
         charge=$(($charge+$pairs))
         $gropath/gmx genion -seed $seed -s ions.tpr -p ${Project}_box_sol.top -nname CL -nq -1 -nn $charge -pname NA -pq 1 -np $pairs -o ${Project}_box_sol_ion.gro 2> addedions << EOF
15
EOF
...
cd $current
cd ../../
initial=$(grep "Initial" $CURRENT_DIR/Infos.dat | awk '{ print $2 }')
